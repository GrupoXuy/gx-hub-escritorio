...
- **Acesso ao GX HUB OS:** `https://gxhubuy.vercel.app/`. O botão **GX HUB OS** aparece para quem possui a permissão **Acesso ao sistema Grupo X**, configurável no painel administrativo.
- **Convites de clientes:** no detalhe de uma reunião, Henrique pode criar um convite temporário de cliente. O convidado preenche nome, sexo, WhatsApp e email e não renderiza o workspace da equipe: com a chamada minimizada ele vê só o andar do escritório (rota própria, `GET /api/guest-office`, que devolve apenas `me`, `members` e `rooms`) e anda com o próprio avatar. O token é de uso único, expira ao fechar a página ou no fim da reunião — trocar de app no celular não encerra a visita — e os dados são armazenados em `gx_leads`; o painel **Leads de clientes** permite pesquisar contatos e exportar CSV.
- **Segurança e diretoria:** a Sala da diretoria é um ambiente separado com mesa de vidro, computador e mobília executiva; apenas Henrique Senna pode entrar. O histórico do chat pode ser apagado pelo administrador. Durante chamadas, a câmera frontal/traseira pode ser alternada no celular e minimizar a janela desliga o vídeo e mantém o áudio (sair da chamada desliga os dois). Trocar de aba ou de app não derruba a chamada: só a destruição da página avisa o servidor e libera a sala.
...
- **Base individual de Leads:** a aba **Leads** fica separada do **Ecossistema Grupo X**. Cada usuário autenticado consulta apenas os leads cujo `owner_id` é seu próprio usuário. Leads antigos de convites são atribuídos ao criador do convite; leads do **GX Radar** entram como `source = gx-radar` e aceitam empresa, WhatsApp, Instagram e site. A interface oferece direcionamento por WhatsApp, Instagram e site e exportação CSV da própria base.
- **Entrada do GX Radar:** o endpoint `POST /api/leads/radar` recebe leads externos usando `GX_RADAR_API_KEY`. O payload deve informar um responsável por `ownerId`/`ownerEmail` (ou aliases `userId`, `assignedTo`, `assignedToEmail`) e pode enviar `name`, `companyName`, `whatsapp`, `email`, `instagram` e `website`. A chave deve existir somente na integração servidor-a-servidor do Radar; nunca no frontend.
